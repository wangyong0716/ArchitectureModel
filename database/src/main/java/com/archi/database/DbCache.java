...
public class DbCache {
    ...
    public DbCache(DbHelper db) {
        ArchiLog.d(SUB_TAG, "new DbCache");
    ...
    public boolean saveDataToDB(InfoHolder data) {
        if (data == null) {
            ArchiLog.d(SUB_TAG, "saveDataToDB To tmp list data == null");
            return false;
        }
                ...
                switch (msg.what) {
                    case MSG_WHAT_PRE_WRITE_DB:
                        ...
                        try {
                            ...
                            ArchiLog.d(SUB_TAG, "class cast exception : " + ex.getMessage());
                        }
                        if (holder != null) {
                            handleData(holder);
                        } else {
                            ArchiLog.d(SUB_TAG, "holder == null");
                        }
                        break;
                    case MSG_WHAT_WRITE_DB:
                            ArchiLog.d(SUB_TAG, "MSG_WHAT_WRITE_DB");
                        updateTime(System.currentTimeMillis());
                        readFromListAndWriteToDB();
                        break;
                    case MSG_WHAT_TIME_OUT:
                            ArchiLog.d(SUB_TAG, "MSG_WHAT_TIME_OUT -> MSG_WHAT_WRITE_DB");
    ...
    private void handleData(InfoHolder data) {
        ...
        ArchiLog.d(SUB_TAG, "saveDataToDB size = " + size + " interval = " + interval + " : name " + data.tableName + " | info : " + data.info.toString());
        ...
        if (interval >= StorageConfig.SAVE_DB_INTERVAL || size >= StorageConfig.SAVE_DB_MAX_COUNT) {
            ArchiLog.d(SUB_TAG, "saveDataToDB To db size = " + size);
    ...
    private void updateTime(long cur) {
        ArchiLog.d(SUB_TAG, "updateTime = " + cur);
        mLastTime = cur;
    }
    ...
    private void readFromListAndWriteToDB() {
        ArchiLog.d(SUB_TAG, "readFromListAndWriteToDB: tid = " + Thread.currentThread().getId() + " start");

        if (myDataList == null || myDataList.isEmpty()) {
            ArchiLog.d(SUB_TAG, "readFromListAndWriteToDB: myDataList empty");
            return;
        }

        SQLiteDatabase db = mDbHelper.getDatabase();
        if (db == null) {
            ArchiLog.d(SUB_TAG, "readFromListAndWriteToDB: db == null");
            return;
        }
        int count = 0;
        ...
        try {
            ...
            if (DEBUG) {
                ArchiLog.d(TAG, SUB_TAG, "readFromListAndWriteToDB beginTransaction");
            }
...
//                    }
            ...
            if (DEBUG) {
                ArchiLog.d(TAG, SUB_TAG, "readFromListAndWriteToDB error\r\n" + Log.getStackTraceString(e));
            }
        } finally {
            if (isBeginTransaction) {
                try {
                    ...
                    ArchiLog.e(TAG, SUB_TAG, "readFromListAndWriteToDB finally error\r\n" + Log.getStackTraceString(e));
                }
            }
            if (DEBUG) {
                ArchiLog.d(TAG, SUB_TAG, "readFromListAndWriteToDB endTransaction count = " + count + " | time = " + (System.currentTimeMillis() - start));
            }
        }
        //db.close();// ???
    }
